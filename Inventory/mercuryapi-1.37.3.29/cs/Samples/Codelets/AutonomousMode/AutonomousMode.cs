...
namespace AutonomousMode
{
    ...
    class AutonomousMode
    {
        ...
        private static void r_tagRead(object sender, TagReadDataEventArgs e)
        {
            if (configOption == "stream")
            {
                Console.WriteLine("Background read: Tag ID:{0} ant:{1} count:{2} time:{3} ", 
                    e.TagReadData.EpcString, getAntennaID(Convert.ToByte(e.TagReadData.Antenna)),
                    e.TagReadData.ReadCount, e.TagReadData.Time.ToString("yyyy-MM-dd'T'HH:mm:ss.fffK"));
            }
            else
            {
            ...
            if (0 < e.TagReadData.Data.Length)
            {
                ...
                {
                    Console.WriteLine("Data[" + (e.TagReadData.dataLength/8) + "]: " + ByteFormat.ToHex(e.TagReadData.Data, "", " "));
                }
            }
        }
        ...
        private static SimpleReadPlan configureUHFPersistentSettings(Reader r, string model, int[] antennaList)
        {
            ...
            //Uncomment this to set BLF, Tari and Tag Encoding for M6e family modules or Network readers
            ...
            if (Reader.Region.OPEN == (Reader.Region)r.ParamGet("/reader/region/id"))
            {
                ...
                //set dwell time
                r.ParamSet("/reader/region/dwellTime", Convert.ToUInt16(250));
                ...
                while (!isConnected)
                {
                    ...
                    foreach (int baudRate in bps)
                    {
                        try
                        {
                            ...
                            serialReader.receiveResponse(_serialPort, ((SerialReader)r).model);
                            isConnected = true;
                            break;
                        }
                        catch (Exception ex)
                        {
                            ...
                            {
                                Console.WriteLine("Failed to connect with " + baudRate + " baudrate.");
                            }
                            else
                            {
                                continue;
