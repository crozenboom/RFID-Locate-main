...
namespace EM4325CustomTagOps
{
    ...
    class EM4325CustomTagOps
    {
        ...
        public class SensorData
        {
            ...
            {
                if ((sensorDataRplyMSW & (1 << 15)) != 0) return (LowBatteryAlarm)(1);
                else return (LowBatteryAlarm)(0);
            }
            //MSW Bit 1
            public AuxAlarm getAuxAlarmStatus(UInt16 sensorDataRplyMSW)
            {
                if ((sensorDataRplyMSW & (1 << 14)) != 0) return (AuxAlarm)(1);
                else return (AuxAlarm)(0);
            }
            //MSW Bit 2
            public OverTempAlarm getOverTempAlarmStatus(UInt16 sensorDataRplyMSW)
            {
                if ((sensorDataRplyMSW & (1 << 13)) != 0) return (OverTempAlarm)(1);
                else return (OverTempAlarm)(0);
            }
            //MSW Bit 3
            public UnderTempAlarm getUnderTempAlarmStatus(UInt16 sensorDataRplyMSW)
            {
                if ((sensorDataRplyMSW & (1 << 12)) != 0) return (UnderTempAlarm)(1);
                else return (UnderTempAlarm)(0);
            }
            //MSW Bit 4
            public P3Input getP3InputStatus(UInt16 sensorDataRplyMSW)
            {
                if ((sensorDataRplyMSW & (1 << 11)) != 0) return (P3Input)(1);
                else return (P3Input)(0);
            }
            //MSW Bit 5
            public MonitorEnabled getMonitorEnabledStatus(UInt16 sensorDataRplyMSW)
            {
                if ((sensorDataRplyMSW & (1 << 10)) != 0) return (MonitorEnabled)(1);
                else return (MonitorEnabled)(0);
            }
            //MSW Bit 6 always 0.
            ...
            public override string ToString()
            {
                ...
                s1 = String.Format("\n lowBatteryAlarmStatus = {0}" +
                                           "\n auxAlarmStatus = {1}" +
                                           "\n overTempAlarmStatus = {2}" +
                                           "\n underTempAlarmStatus = {3}" +
                                           "\n p3InputStatus = {4}" +
                                           "\n monitorEnabledStatus = {5}",
                                           lowBatteryAlarmStatus,
                                           auxAlarmStatus,
                                           overTempAlarmStatus,
                                           underTempAlarmStatus,
                                           p3InputStatus,
                                           monitorEnabledStatus
                                           );
                ...
                s3 = String.Format("\n abortedTemperatureCount = {0}" +
                                           "\n underTemperatureCount = {1}" +
                                           "\n overTemperatureCount = {2}",
