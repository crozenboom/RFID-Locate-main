...
//Uncomment this to enable setting access password
//#define ENABLE_M3E_SETACCESSPWD
                ...
                using (r = Reader.Create(args[0]))
                {
...
#if ENABLE_READ_AFTER_WRITE
                    {
                        ...
                        //Write one word of data to USER memory and read back 8 words from EPC memory using WriteData and ReadData
                        {
                            ...
                            response = (ushort[])r.ExecuteTagOp(tagopList, null);
                            Console.WriteLine("ReadData after WriteData is successful.");
                        ...
                        //Write 12 bytes(6 words) of EPC and read back 8 words from EPC memory using WriteTag and ReadData
                        {
                            ...
                            // call executeTagOp with list of tagops
                            response = (ushort[])r.ExecuteTagOp(tagopList, null);
                            Console.WriteLine("ReadData after WriteTag is successful.");
                            Console.WriteLine("ReadData: ");
                            foreach (ushort word in response)
                            {
                                Console.Write(" {0:X4}", word);
                    ...
                    type = MemoryType.TAG_MEMORY;
                    address = 0;
                    length = 1;

                    // Read memory before write
                    ReadMemory bRead = new ReadMemory(type, address, length);
#if ENABLE_M3E_SETACCESSPWD
                    // Enable and set the password before execute tag op

                    //Set password
                    byte[] accessPwd = new byte[] { 0xff, 0xff, 0xff, 0xff };
                    r.SetAccessPassword(bRead, accessPwd);
                    ...
                    type = MemoryType.PROTECTION_SECURITY_STATUS;
                    address = 0;
                    length = 1;
