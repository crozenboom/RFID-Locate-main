    ...
    class GpioCommands
    {
        static void Usage()
        {
            Console.WriteLine(String.Join("\r\n", new string[] {
                    ...
                    " 2) set-gpo -- Write output pin(s)", " set-gpo [[pin, value],...] where pin -- Pin number, value -- Pin state:1 to set pin high, 0 to set pin low",
                    " set-gpo [[2,1],[3,1]]",
                    " 3) testgpiodirection -- verifying gpio directionality"
                }));
            Environment.Exit(1);
        }
        ...
        private static GpioPin[] ArrayListToGpioPinArray(ArrayList list)
        {
            ...
            foreach (ArrayList innerlist in list)
            {
                ...
                {
                    throw new ArgumentException("Can't parse " + pinval[1] + " as GPO state. Allowed values are 0(low) or 1(high).");
                }
