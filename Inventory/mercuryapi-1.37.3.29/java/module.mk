...
MERCURYAPI_JAVA_TARGETS  := $(MERCURYAPI_JAVA_BIN_TARGET)
ifndef SKIP_SAMPLES
#MERCURYAPI_JAVA_TARGETS  := $(BUILD)/demo.jar
endif
...
NATIVELIBS  += $(JNILIBDIR)/linux-arm.lib
NATIVELIBS  += $(JNILIBDIR)/linux-aarch64.lib
...
TARGETS  += $(BUILD)/BlockWrite.class $(BUILD)/MultiProtocol.class $(BUILD)/multireadasync.class
TARGETS  += $(BUILD)/writetag.class $(BUILD)/EmbeddedReadTID.class
endif
...
# Targets
#
$(MERCURYAPI_JARFILE): $(BUILD)/manifest $(MERCURYAPI_JAVA_OBJS) $(NATIVELIBS) $(MERCURYAPI_JAVA_NETBEANS_SRCDIR)/log4j.properties $(MERCURYAPI_JAVA_NETBEANS_SRCDIR)/log4j2.xml
	cp $(MERCURYAPI_JAVA_NETBEANS_SRCDIR)/log4j.properties $(BUILD)
	cp $(MERCURYAPI_JAVA_NETBEANS_SRCDIR)/log4j2.xml $(BUILD)
	(cd $(BUILD); jar -cfm $(notdir $@) manifest log4j.properties log4j2.xml $(MERCURYAPI_JAVA_SUBDIR)/*.class $(subst $(BUILD)/,,$(NATIVELIBS)))
...
$(MERCURYAPI_JAVA_DIR)/linux-amd64.lib:
	$(error Autobuild not yet implemented for $@.  See tm/modules/mercuryapi/c/proj/jni/buildbox-linux)

$(MERCURYAPI_JAVA_DIR)/linux-x86.lib:
	$(error Autobuild not yet implemented for $@.  See tm/modules/mercuryapi/c/proj/jni/buildbox-linux)
